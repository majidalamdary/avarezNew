...
public class adapter_pasmand extends RecyclerView.Adapter<adapter_pasmand.my_view_holder> {
    private Context context;
    private List<item_pasmand> item;
    ...
    String
            font_name = "";
    Typeface tf;
    public adapter_pasmand(Context context, List<item_pasmand> item) {
    ...
    public my_view_holder onCreateViewHolder(ViewGroup parent, int viewType) {
       View view= LayoutInflater.from(context).inflate(R.layout.list_pasmand_item,parent,false);
    ...
    public void onBindViewHolder(final my_view_holder holder, final int position) {
        ...
        holder.txt_type.setText(item.get(position).getGroupName());
        holder.txt_unit.setText(item.get(position).getUnit());
